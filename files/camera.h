...
public:
    ...
    void Pan ( Camera_Movement direction, GLfloat amount)
    {
        GLfloat velocity = this-> movementSpeed * amount;
        // If statements to control movement
        // If is used instead of elses because we want to be able to move in two directions at once
    ...
    void Scrollpan (float x, float y, float z)
    {
        // Pan the camera using middle mouse drag
        this-> position += this -> right *x*this->movementSpeed;
        this-> position += this -> up *-y*this->movementSpeed;
        this-> position += this -> front *z*this->movementSpeed;
    }
